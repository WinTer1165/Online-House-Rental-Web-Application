...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("SELECT user_id, phone FROM Users WHERE username = ?");
    ...
    if ($stmt->num_rows == 1) {
        ...
        $expires = time() + 600; // Code expires in 10 minutes
        ...
        // Send the verification code via SMS using cURL
        // Replace with your actual Twilio credentials
        $account_sid = $config['twilio']['account_sid'];
        $auth_token = $config['twilio']['auth_token'];
        $twilio_number = $config['twilio']['phone_number'];// e.g., '+1234567890'
